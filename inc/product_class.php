<?php
	...
	function cfwc_create_custom_field() {
		$args = array(
			...
			'label'         => __( 'Product Class', 'wcpf' ),
			'class'			=> 'cfwc-custom-field',
			'desc_tip'      => true,
			'description'   => __( 'Enter Product Class.', 'wcpf' ),
		);
	...
	function cfwc_display_custom_field() {
		...
		if( $title != "" ) {
			echo '<div class="cfwc-custom-field-wrapper"><input type="hidden" id="cfwc-title-field" name="cfwc-title-field" value="'.$title.'"></div>';
	...
	function cfwc_add_custom_data_to_order( $item, $cart_item_key, $values, $order ) {
		foreach( $item as $cart_item_key=>$values ) {
			if( isset( $values['title_field'] ) ) {
				$item->add_meta_data( __( 'product_class', 'wcpf' ), $values['title_field'], true );
			}
		}
	}
